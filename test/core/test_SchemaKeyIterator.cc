...
CASE("Iterate list of keys") {
    ...
    struct Visitor {
        ...
        int level3 = -1;
        int count = 0;
        const std::vector<dasi::Key>& seq1_;
        const std::vector<dasi::Key>& seq2_;
        const std::vector<dasi::Key>& seq3_;
        Visitor(const std::vector<dasi::Key>& seq1,
                const std::vector<dasi::Key>& seq2,
                const std::vector<dasi::Key>& seq3) :
                seq1_(seq1), seq2_(seq2), seq3_(seq3) {}
        void firstLevel(const dasi::SplitReferenceKey& key) {
            level1++;
            EXPECT(key[0] == seq1_[level1]);
            level2 = -1;
            level3 = -1;
        }
        void secondLevel(const dasi::SplitReferenceKey& key) {
            level2++;
            EXPECT(key[0] == seq1_[level1]);
            EXPECT(key[1] == seq2_[level2]);
            level3 = -1;
        }
        void thirdLevel(const dasi::SplitReferenceKey& key) {
            level3++;
            count++;
            EXPECT(key[0] == seq1_[level1]);
            EXPECT(key[1] == seq2_[level2]);
...
CASE("Can handle non-list iteration (i.e. key vs request)") {
    ...
    dasi::Key lev1 {{"k1", "aa"}, {"k2", "dd"}};
    dasi::Key lev2 {{"k3", "gg"}, {"k4", "jj"}};
    dasi::Key lev3 {{"k5", "mm"}, {"k6", "pp"}};
    ...
    struct Visitor {
        ...
        bool visited3 = false;
        int count = 0;
        const dasi::Key& lev1_;
        const dasi::Key& lev2_;
        const dasi::Key& lev3_;
        Visitor(const dasi::Key& lev1, const dasi::Key& lev2, const dasi::Key& lev3) :
