...
int main(int argc, char **argv) {
    ...
    for (char** argp = argv + 1; *argp != NULL; ++argp) {
        if (strcmp(*argp, "-c") == 0 || strcmp(*argp, "--config") == 0) {
            ...
            fprintf(stderr, "Usage: mean [--config PATH] [--version VERS] [--date YYYYMMDD] [--time HHMM] [--members NUM] [--levels NUM] [--step NUM]\n");
            return 1;
        }
    }
    ...
    for (int level = 0; level < num_levels; ++level) {
        ...
        for (int par = 0; par < num_params; ++par) {
            ...
            if (dasi_get(dasi, query, &result) != DASI_SUCCESS) {
                fprintf(stderr,
                    "Could not find step=%d, level=%d, param=%s\n",
            ...
            do {
                ...
                if (sscanf(data, "%lg", &val) != 1) {
                    fprintf(stderr,
                        "Could not read data number=%d, step=%d, level=%d, param=%s\n",
